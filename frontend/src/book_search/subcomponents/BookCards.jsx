...
export default function({books_to_show, current_page, change_current_page}) {
    ...
    if(books_to_show.length != 0) {
        ...
        let current_books = books_to_show.slice(current_page*6-6, current_page*6);

        return (
        <div className="grid gap-4">
            <div className="grid grid-cols-3 grid-rows-2 gap-4">
                {current_books.map((item)=> <Card name={item.name} author={item.author} rating={item.rating} genre={item.genre}/>)}
            </div>
